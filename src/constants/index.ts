...
export const isValidUserRole = (role: string): role is typeof USER_ROLES[keyof typeof USER_ROLES] => {
  return VALID_USER_ROLES.includes(role as typeof USER_ROLES[keyof typeof USER_ROLES])
}

export const isValidRequestStatus = (status: string): status is typeof REQUEST_STATUS[keyof typeof REQUEST_STATUS] => {
  return VALID_REQUEST_STATUSES.includes(status as typeof REQUEST_STATUS[keyof typeof REQUEST_STATUS])
}
